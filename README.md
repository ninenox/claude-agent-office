...
```
python main.py
  (ไม่มี flag)       server + agents พร้อมกัน (default tasks)
  --server-only      server อย่างเดียว (demo mode)
  --agents-only      agents อย่างเดียว (ไม่มี UI)
  --tasks <file>     ระบุไฟล์ JSON กำหนด tasks
